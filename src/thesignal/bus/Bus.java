...
public class Bus {
	...
	public void handle(Command command) {
		...
		CommandHandler<Command> commandHandler = (CommandHandler<Command>) commandHandlers
			.get(command.getClass().getName());
		if (commandHandler == null) {
			logger.info("No handler for \"" + command.getClass().getName()
					+ "\" registered.");
			return;
		}
		commandHandler.handle(command, this);
		logger
			.info("Command \"" + command.getClass().getName() + "\" handled.");
	}

	@SuppressWarnings("unchecked")
	public void raise(Event event) {
		logger.info("Raising event \"" + event.getClass().getName() + "\" ...");
		HashSet<EventListener<?>> listeners = eventListeners.get(event
			.getClass()
			.getName());
		if (listeners == null) {
			logger.info("No listeners for \"" + event.getClass().getName()
					+ "\" registered.");
			return;
		}
	...
	public void register(CommandHandler<?> commandHandler, String commandName)
		...
		if (commandHandler.equals(oldHandler)) {
			...
			throw new RegisterException(
					"Another handler was already registered for the command "
							+ commandName + "!");
		}
	...
	public void unregister(CommandHandler<?> commandHandler, String commandName)
		...
		if (!commandHandlers.containsKey(commandName)) {
			throw new UnregisterException("Command " + commandName
					+ " not registered!");
		}
		if (commandHandlers.get(commandName).equals(commandHandler)) {
			throw new UnregisterException(
					"Another commandhandler was registred!");
		}
	...
	public void register(EventListener<?> eventListener, String eventName)
		...
		if (listeners.contains(eventListener)) {
			throw new RegisterException("Reregistering same eventListener \""
					+ eventListener.getClass().getName() + "\" for Event \""
					+ eventName + "\"");
		}
	...
	public void unregister(EventListener<?> eventListener, String eventName)
		...
		HashSet<EventListener<?>> listeners = eventListeners.get(eventName);
		if (listeners == null) {
			throw new UnregisterException("Event " + eventName
					+ " not registered!");
		}
		if (!listeners.remove(eventListener)) {
			throw new UnregisterException(eventListener.getClass().getName()
					+ " not registered for event " + eventName + "!");
		}
	}
}
